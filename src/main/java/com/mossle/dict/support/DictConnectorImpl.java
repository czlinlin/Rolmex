...
public class DictConnectorImpl implements DictConnector {
    ...
    public List<DictInfo> findDictInfoListByType(String typeName, String tenantId) {
        DictType dictType = dictTypeManager
                .findUnique("from DictType where name=? and tenantId=?",
                        typeName, tenantId);
        DictDTO dictDto = new DictDTO();
        ...
        List<DictInfo> dictInfos = dictInfoManager.find(hql,
                dictType.getId());

        return dictInfos;
    }
    
    public List<DictInfo> findDictInfoListByType(String typeName) {
        DictType dictType = dictTypeManager
                .findUnique("from DictType where name=?",
                        typeName);
        DictDTO dictDto = new DictDTO();
        dictDto.setType(dictType.getName());
    ...
    public String findDictNameByValue(String typeName,String value)
    {
    	DictType dictType = dictTypeManager
                .findUnique("from DictType where name=?",
                        typeName);

        String hql = "from DictInfo where dictType.id=? and value=? order by priority";
        DictInfo dictInfo = dictInfoManager.findUnique(hql,
                dictType.getId(),value);
    	if(dictInfo!=null)
