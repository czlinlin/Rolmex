...
public class TaskDefinitionConnectorImpl implements TaskDefinitionConnector {
	...
	public List<TaskUserDTO> findTaskUsers(String taskDefinitionKey,
        ...
        // 如果TaskDefUser中找不到数据 就到 BpmConfUser中找数据   zyl 2018-09-19
        if (taskUserDtos.isEmpty()) {
        	hql = "from BpmConfBase where processDefinitionId=?";
        	BpmConfBase bpmConfBase = bpmConfBaseManager.findUnique(hql, processDefinitionId);
        	if (bpmConfBase != null) {
	        	hql = "from BpmConfUser where bpmConfNode.code=? and bpmConfNode.bpmConfBase=?";
	            List<BpmConfUser> bpmConfUsers = bpmConfUserManager.find(hql,
	                    taskDefinitionKey, bpmConfBase);
	            
	            for (BpmConfUser bpmConfUser : bpmConfUsers) {
	                
	                TaskUserDTO taskUserDto = new TaskUserDTO();
	                
	                if (bpmConfUser.getType() == 0) {   // 负责人   
	                	taskUserDto.setCatalog("assignee");
	                } else if ((bpmConfUser.getType() == 1) || (bpmConfUser.getType() == 2)) {  // 候选人  候选组
	                	taskUserDto.setCatalog("candidate");
	                } else if (bpmConfUser.getType() == 3) {   // 抄送人
	                	taskUserDto.setCatalog("notification");
	                } else if (bpmConfUser.getType() == 4) {   // 大区候选组      // 数据权限流程用
	                	taskUserDto.setCatalog("areacandidate");
	                } else if (bpmConfUser.getType() == 5) {   // 同一大区         // 发起人和审批人是同一大区的审核
	                	taskUserDto.setCatalog("sameareacandidate");
	                } else if (bpmConfUser.getType() == 6) {   // 同一分公司         // 发起人和审批人是同一分公司的审核
	                	taskUserDto.setCatalog("samecompanycandidate");
	                }

	                if ((bpmConfUser.getType() == 0) || (bpmConfUser.getType() == 1)) {
	                	taskUserDto.setType("user");
	                } else if ((bpmConfUser.getType() == 2) || (bpmConfUser.getType() == 4) || 
	                		(bpmConfUser.getType() == 5) || (bpmConfUser.getType() == 6)) {
	                	taskUserDto.setType("group");
	                }

	                taskUserDto.setValue(bpmConfUser.getValue());
	                taskUserDtos.add(taskUserDto);
	            }
        	}
        }
    ...
    public void saveForm(String taskDefinitionKey, String processDefinitionId,
        ...
        if (form.getKey() != null && !form.getKey().equals("")) {
    ...
    public void addTaskUser(String taskDefinitionKey,
        ...
        taskDefUser.setName(taskUser.getName());
        taskDefUserManager.save(taskDefUser);
    }
