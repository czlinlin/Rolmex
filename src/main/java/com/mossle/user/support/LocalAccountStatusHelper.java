...
public class LocalAccountStatusHelper implements AccountStatusHelper {
    ...
    public boolean findAccountLocked(String accountId) {
        AccountInfo accountInfo = accountInfoManager.findUnique(
                "from AccountInfo where id=?", Long.valueOf(accountId));
        if(accountInfo==null)
        	return false;
        else{
        	if(accountInfo.getLocked().toLowerCase().equals("locked")){
        		return true;
        	}
        	else
        		return false;
    ...
    public String getAccountStatus(String username, String application) {
        ...
        if (accountInfo.equals(PersonInfoConstants.DELETE_FLAG_YES)) {
        	return AccountStatus.ACCOUNT_NOT_EXISTS;
        }
