...
public class DatabaseUserConnector implements UserConnector {
    ...
    private String sqlFindById = "SELECT AI.ID AS ID,AI.USERNAME AS USERNAME,AI.STATUS AS STATUS,"
            ...
            + "PI.CELLPHONE AS MOBILE,1 AS USER_REPO_REF,PI.POSITION_CODE"
            + " FROM ACCOUNT_INFO AI LEFT JOIN PERSON_INFO PI ON AI.CODE=PI.CODE OR AI.ID=PI.ID"
            + " WHERE AI.DEL_FLAG = '0' AND AI.ID=?";

    private String sqlFindByIdAll = "SELECT AI.ID AS ID,AI.USERNAME AS USERNAME,AI.STATUS AS STATUS,"
            + "AI.NICK_NAME AS NICK_NAME,AI.DISPLAY_NAME AS DISPLAY_NAME,PI.EMAIL AS EMAIL,"
            + "PI.CELLPHONE AS MOBILE,1 AS USER_REPO_REF,"
            + "AI.DEL_FLAG AS DEL_FLAG"//sjx
    ...
    private String sqlFindByUsername = "SELECT AI.ID AS ID,AI.USERNAME AS USERNAME,AI.STATUS AS STATUS,"
            ...
            + " FROM ACCOUNT_INFO AI LEFT JOIN PERSON_INFO PI ON AI.CODE=PI.CODE"
            + " WHERE AI.DEL_FLAG= '0' AND AI.USERNAME=? AND AI.TENANT_ID=?";  //ckx AI.DEL_FLAG= '0' AND
    private String sqlFindByRef = "SELECT UB.ID AS ID,UB.USERNAME AS USERNAME,UB.STATUS AS STATUS,"
            + "NICK_NAME AS NICK_NAME,EMAIL AS EMAIL,MOBILE AS MOBILE,USER_REPO_ID AS USER_REPO_REF"
            + " FROM ACCOUNT_INFO UB WHERE UB.DEL_FLAG = '0' AND UB.REF=? AND UB.USER_REPO_ID=?";
    private String sqlPagedQueryCount = "SELECT COUNT(*) FROM ACCOUNT_INFO AI WHERE AI.DEL_FLAG = '0'";
    ...
    private String sqlFindByNickName = "SELECT ID AS ID,USERNAME AS USERNAME,STATUS AS STATUS,"
            + "NICK_NAME AS NICK_NAME,EMAIL AS EMAIL,MOBILE AS MOBILE,USER_REPO_ID AS USER_REPO_REF"
            + " FROM ACCOUNT_INFO WHERE NICK_NAME=? AND DEL_FLAG = '0'";
    ...
    public UserDTO findByIdAll(String id) {
        ...
        try {
        	if (!StringUtils.isBlank(id)) {
	            Map<String, Object> map = jdbcTemplate.queryForMap(sqlFindByIdAll, id);
    ...
    protected UserDTO convertUserDTO(Map<String, Object> map) {
        ...
        userDTO.setDelFlag(convertString(map.get("del_flag")));
        userDTO.setPositionCode(convertString(map.get("position_code")));
