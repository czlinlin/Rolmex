...
public class DatabaseUserSyncConnector implements UserSyncConnector {
    ...
    private String sqlFindByCode = "select count(*) from ACCOUNT_INFO where code=? AND DEL_FLAG ='0'";
    private String sqlUpdate = "update ACCOUNT_INFO set username=?,nick_name=?,display_name=?,status='active' where code=?";
    private String sqlInsert = "insert into ACCOUNT_INFO(code,username,nick_name,display_name,status,del_flag) values(?,?,?,?,'active','0')";
