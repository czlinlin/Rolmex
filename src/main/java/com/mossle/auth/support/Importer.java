...
public class Importer {
    ...
    public void executeSql() {
        ...
        for (Map<String, Object> user : userList) {
            ...
            try {
                jdbcTemplate
                        .update("insert into auth_user_role(user_status_id,role_id,del_flag) values(?,?,?)",
                                userId, roleCache.get(user.get("role")),'0');
            } catch (Exception ex) {
    ...
    public Long createOrGetUserId(String username, String reference,
            String tenantId) {
        if (this.notExists(
                "select count(*) form auth_user_status where reference=? and tenant_id=? and del_flag = '0'",
                reference, tenantId)) {
            jdbcTemplate
                    .update("insert into auth_user_status(username,reference,status,tenant_id,del_flag) values(?,?,1,?,'0')",
                            username, reference, tenantId);
        }

        return jdbcTemplate
                .queryForObject(
                        "select id from auth_user_status where reference=? and tenant_id=? and del_flag='0'",
