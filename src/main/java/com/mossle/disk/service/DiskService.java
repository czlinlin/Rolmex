...
public class DiskService {
	 ...
	 * 上传文件.
	 */
	public DiskInfo createFile(String userId, DataSource dataSource, String name, long size, String parentPath,
			String tenantId) throws Exception {
		String modelName = "disk/user/" + userId;
		String keyName = parentPath + "/" + name;
		StoreDTO storeDto = storeConnector.saveStore(modelName, keyName, dataSource, tenantId);
	...
	public DiskInfo createDir(String userId, String name, String parentPath) {
		internalStoreConnector.mkdir("1/disk/user/" + userId + "/" + parentPath + "/" + name);
	 ...
	 * 上传文件，或新建文件夹.
	 */
	public DiskInfo createDiskInfo(String userId, String name, long size, String ref, String type, int dirType,
			String parentPath) {
		if (name == null) {
	...
	public String move(String userId, Long id, Long parentId) {
		...
		if (parentId != 0) {
			...
			if ("dir".equals(diskInfo.getType()) && checkedParentPath.startsWith(currentPath)) {
				logger.info("{}({}) is sub directory of {}({})", parent.getParentPath() + "/" + parent.getName(),
						parentId, diskInfo.getParentPath() + "/" + diskInfo.getName(), id);
