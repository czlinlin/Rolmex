	...
	<script type="text/javascript">
        ...
        function searchInfo() {
        	...
        	if(startDate != "" && endDate != ""){
        		if(startDate == endDate){
            		title = contractCompanyName+"公司"+startDate+"公积金扣款明细";
            	}else{
            		title = contractCompanyName+"公司"+startDate+"~"+endDate+"公积金扣款明细";
            	}
        	}else if(startDate == ""){
        		alert("请选择开始时间");
        		return false;
        	...
        	$("#jqGrid-salary-table").jqGrid('setGridParam',{ 
                url:"${tenantPrefix}/user/salary-accumulation-fund-list-data.do", 
                mtype:"post",
                datatype : "json", 
                postData:{
                	'postId':postId,
		...
		function importSalary(){
			$('#salaryBaseForm').attr('action', '${tenantPrefix}/user/salary-accumulation-fund-import-excel.do');
		}
		//生成当月数据
		function createMonthData(){
			...
			//查询当前薪资单位下的最大月份
			$.ajax({  
                url : "${tenantPrefix}/user/queryAccumulationFundMonthByContractCompanyId.do",  
                data : {  
                    "contractCompanyId" : contractCompanyId  
                },  
                type : 'post', 
                ...
                success : function(data) { 
                	if(data.boo){
                    	...
                    	if (confirm("是否生成"+year+"年"+month+"月的数据？")){
                    		//生成数据
                			$.ajax({  
                                url : "${tenantPrefix}/user/salaryAccumulationFundCreateMonthData.do",  
                                data : {  
                                    "contractCompanyId" : contractCompanyId,
		...
		$(document).ready(function () {
			$.ajax({
				...
				dataType : 'json',
				data:{
					url:"/user/salary-accumulation-fund-list-i.do"
				},
				success :function(data){
		...
		function showTable(data){
	         $("#jqGrid-salary-table").jqGrid({
	                ...
	                colNames: ['操作','员工状态','月份','姓名', '身份证号','缴存基数',  '比例', '金额', '比例','金额','合计'],
	                colModel: [
						{ name : 'act',align: 'center',width : 130 }, 
						{ label: '员工状态', name: 'quitFlag',align: 'center', width: 130 },
						{ label: '月份', name: 'accumulationFundDate',align: 'center', width: 130 ,sortable: false},
						{ label: '姓名', name: 'personName',align: 'center', width: 130 ,sortable: false},
						{ label: '身份证号', name: 'idcardNum',align: 'center', width: 200, editable: true,cellattr: addCellAttr,sortable: false},
						{ label: '缴存基数', name: 'accumulationFundBaseMoney',align: 'center', width: 130, editable: true,cellattr: addCellAttr,sortable: false},
						{ label: '比例', name: 'accumulationFundCompanyProportion',align: 'center', width: 130,editable: true,cellattr: addCellAttr,sortable: false},
						{ label: '金额', name: 'accumulationFundCompanyMoney',align: 'center', width: 130,editable: true ,cellattr: addCellAttr,sortable: false},
						{ label: '比例', name: 'accumulationFundPersonalProportion',align: 'center', width: 130,editable: true,cellattr: addCellAttr,sortable: false},
						{ label: '金额', name: 'accumulationFundPersonalMoney',align: 'center', width: 130,editable: true,cellattr: addCellAttr,sortable: false},
						{ label: '合计', name: 'totalMoney',align: 'center', width: 155,editable: true,cellattr: addCellAttr,sortable: false},
	                ],
					loadonce: true,
		...
		function setHeader(){
            ...
            jQuery("#jqGrid-salary-table").jqGrid('setGroupHeaders', { 
            	useColSpanStyle: true, 
            	groupHeaders:[ 
            	               {startColumnName: 'accumulationFundCompanyProportion', numberOfColumns: 2, titleText: '单位'},
            	               {startColumnName: 'accumulationFundPersonalProportion', numberOfColumns: 2, titleText: '个人'}
            	             ] 
            });
		...
		function addCellAttr(rowId, val, rawObject, cm, rdata) {
		 ...
		 function saveParam(rowId) { 
			 var parameter = { 
					url : "${tenantPrefix}/user/editSalaryAccumulationFund.do", //代替jqgrid中的editurl 
					mtype : "POST", 
				...
				//var ids = jQuery("#jqGrid-salary-table").jqGrid('getDataIDs'); 
           		...
           		/* var url = '${tenantPrefix}/user/salary-accumulation-fund-list-data.do';  
                $("#jqGrid-salary-table").jqGrid('setGridParam', {  
                    datatype : "json",  
                    url : url  
                }).trigger("reloadGrid"); */
		...
		function deleteRow(id){
			if (confirm("您确定删除此记录？")){ 
				$.ajax({  
	                url : "${tenantPrefix}/user/deleteSalaryAccumulationFundById.do",  
	                data : {  
	                    "id" : id  
	                },  
	                type : 'post', 
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form id="salaryBaseForm" class="form-inline">
                    ...
                    <button id="btn_export" style="display:none;" class="btn btn-default a-search" onclick="exportAccumulationLundForm()">导出</button>
...
<script type="text/javascript">
	/* 公积金导出  */
	function exportAccumulationLundForm(){
		if($("#startDate").val()=="" || $("#endDate").val()==""){
			alert("请选择日期范围");
			return false;
		}
		$("#salaryBaseForm").attr("action","accumulation-fund-export.do");
	}
</script>
</body>
</html>
