...
// 	$('#dictTypeForm').attr('action', '${tenantPrefix}/user/contract-company-manage-save.do');
// 	$('#dictTypeForm').submit();
// }
...
<form id="dict-typeForm" method="post" action="contract-company-manage-save.do" class="form-horizontal">
	...
	<input id="id" type="hidden" name="id" value="${model.id}">
	  ...
	  <div class="form-group">
		...
		<div class="col-sm-5">
		  <input id="contractCompanyName" type="text" name="contractCompanyName" value="${model.contractCompanyName}" size="40" class="form-control required">
	    </div>
	  </div>
	  
	  <div class="form-group">
	    <label class="control-label col-md-1" ><font color="red">*</font>单位邮箱</label>
		<div class="col-sm-5">
		  <input id="companyEmail" type="text" name="companyEmail"  value="${model.companyEmail}"   class="form-control required email" >
	    </div>
	  </div>
	  
	  <div class="form-group">
	    <label class="control-label col-md-1" ><font color="red">*</font>smtp服务器</label>
		<div class="col-sm-5">
		  <select name="smtpServerId" id="smtpServerId" class="form-control">
		  	<c:forEach items="${smtpServerList }" var="smtpServer">
		  		<option value="${smtpServer.id}" ${model.smtpServer.id==smtpServer.id?"selected='selected'":""}>${smtpServer.name }</option>
		  	</c:forEach>
		  </select>
	    </div>
	  </div>
	  ...
	  <div class="form-group">
	    <label class="control-label col-md-1" ><font color="red">*</font>pop服务器</label>
		<div class="col-sm-5">
		   <select name="popServerId" id="popServerId" class="form-control">
		  	<c:forEach items="${popServerList }" var="popServer">
		  		<option value="${popServer.id}" ${model.popServer.id==popServer.id?"selected='selected'":""}>${popServer.name }</option>
		  	</c:forEach>
		   </select>
		</div>
	  </div>
		...
		<div class="col-sm-5">
		  <input id="remark" type="text" name="remark"   value="${model.remark}" size="40"  class="form-control">
	    </div>
	  </div>
