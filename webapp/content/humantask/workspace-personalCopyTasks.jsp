...
<head>
    ...
    <script type="text/javascript">
        var config = {
            ...
            exportUrl: 'personalCopyTasks-export.do'
        };
        ...
        function fnSearch() {
            document.getElementById("charege-infoForm").action = "workspace-personalCopyTasks.do";
        }
        function fnGoAction() {
            ...
            document.getElementById("charege-infoForm").action = "personalCopyTasks-export.do";
        }
        ...
        function copyDetail(processInstanceId,obj){
        	var rel = $(obj).attr("rel");
        	
        	$.ajax({
                url: "${tenantPrefix}/operationCustom/copyUpdate.do",
                dataType:'json',
                type: "POST",
                ...
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    consol.log("[" + XMLHttpRequest.status + "]error，请求失败")
                },
                complete: function (xh, status) {
                	consol.log("请求超时");
                }
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <form id="charege-infoForm" name="charege-infoForm" method="post"
                      action="workspace-personalCopyTasks.do"
                      class="form-inline">
                    <div class="row">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            <label >状态:</label>
                            <select id="status" class="form-control" name="status" title="">
                                <option value="2" <c:if test="${status=='2'}">selected='selected'</c:if>>全部</option>
                                <option value="1" <c:if test="${status=='1'}">selected='selected'</c:if>>已读</option>
                                <option value="0" <c:if test="${status=='0'}">selected='selected'</c:if>>未读</option>
                            </select></div>
                    </div>
                    <br>
                    <div class="row">
        ...
        <form id="pimRemindGridForm" name="pimRemindGridForm" method='post' action="pim-note-remove.do"
            ...
            <div class="panel panel-default">
                ...
                <div style="overflow-x:scroll">
                    <table id="pimRemindGrid" class="table table-hover mytable" style="min-width:2000px;">
                        <thead>
                        <tr>
                            <th width="8%">操作</th>
                            <th>受理单号</th>
                        ...
                        <c:forEach items="${page.result}" var="item">
                            ...
                            <tr  <c:if test="${item.businessDetailId != '8001' or item.businessDetailId == '8001'}"></c:if> <c:if test="${item.businessDetailId == '8001' and item.compare==false}">style="color:red;"</c:if>>
                                <td>
                                    ...
                                    <!-- 表单跳转路径更改为，无论是自定义还普通流程，点击详情直接跳表单 原路径为 /bpm/workspace-viewHistory.do-->
                                    <a <%-- onclick="copyDetail(${item.processInstanceId},this)" --%>
                                       href="${tenantPrefix}/bpm/workspace-viewHistoryFrom.do?processInstanceId=${item.processInstanceId}&url=${item.url}"
                                       target="_blank">详情</a>
                                ...
                                <td><c:if test="${!fn:contains(item.url,'?')}">
                                    <a  class="workTask_title rwop" title="${item.theme}"
                                        ...
                                        <%-- onclick="copyDetail(${item.processInstanceId},this)" --%>
                                ...
                                <td><fmt:formatDate value="${item.ccTime}"
