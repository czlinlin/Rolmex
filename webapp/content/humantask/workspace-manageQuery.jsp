...
<%pageContext.setAttribute("currentChildMenu", "管理者");%>
<!doctype html>
<html lang="en">
...
<head>
	...
	<script type="text/javascript" src="${cdnPrefix}/business/stepDetail.js"></script>
    <script type="text/javascript">
        var config = {
            ...
            isBranches:"${tenantPrefix}/dict/isBranches.do",
            detailShowUrl:"${tenantPrefix}/dict/getProcessPostInfoByBusinessDetailId.do",
            detailShowCustomUrl:"${tenantPrefix}/dict/getProcessPostInfoByProcessInstanceIdCustom.do",
            branchesUrl:"${tenantPrefix}/dict/getBranchProcessStep.do"
        };
        ...
        function fnSearch() {
            document.getElementById("charege-infoForm").action = "workspace-manageQuery.do";
        }
        function fnGoAction() {
            ...
            document.getElementById("charege-infoForm").action = "manageQuery-export.do";
        }
    ...
    <section id="m-main" class="col-md-10" style="padding-top:65px;">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <form id="charege-infoForm" name="charege-infoForm" method="post" action="workspace-manageQuery.do"
                      class="form-inline">
                    <div class="row">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="workTask_Status" class="form-control" name="filter_EQS_pro_status" title="">
                                ...
                                <c:forEach items="${statuslist}" var="item">
                                	<c:if test="${item.value != 2 and item.value != 3 and item.value != 6}">
                    ...
                    <table id="pimRemindGrid" class="table table-hover mytable"
                        ...
                        <c:forEach items="${page.result}" var="item">
                            ...
                            <tr  <c:if test="${item.status == '未审核'|| item.status == '驳回发起人'}"> style="color:blue"  </c:if>  <c:if test="${item.status == '审核中'||item.status == '审核中（驳回）'}"> style="color:green" </c:if>
                                ...
                                <c:if test="${item.businessTypeId != '9999'}">
                                	<td id="whole"><a href="javascript:"onclick='detailShow("${item.businessDetailId}","${item.businessKey}")'>[查看]</a></td>
                                </c:if>
                                <c:if test="${item.businessTypeId == '9999'}">
                                	<td id="whole"><a href="javascript:"onclick='detailShowCustom("${item.processInstanceId}")'>[查看]</a></td>
                                </c:if>
