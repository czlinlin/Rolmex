...
%>
<%
    pageContext.setAttribute("currentChildMenu", "已办任务");
%>
<!doctype html>
<html lang="en">
...
<head>
    ...
    <script type="text/javascript"
            src="${cdnPrefix}/userpicker3-v2/userpicker.js"></script>
    <script type="text/javascript">
        var config = {
            id: 'processGrid',
            pageNo: ${page.pageNo},
        ...
        function fnSearch() {
            document.getElementById("charege-infoForm").action = "workspace-historyTasks.do";
        }
        function fnGoAction() {
            ...
            document.getElementById("charege-infoForm").action = "historyTasks-export.do";
        }
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <form id="charege-infoForm" name="charege-infoForm" method="post"
                      action="workspace-historyTasks.do" class="form-inline">
                    <div class="row">
                        <div class="col-md-4">
                            <label>受理单号:</label> <input type="text" style="width: 200px;"
                                                        maxlength="30" id="charege-info_applyCode"
                                                        name="filter_LIKES_applyCode"
                                                        value="${param.filter_LIKES_applyCode}" class="form-control"
                                                        onchange="fntrim(this)">
                        </div>
                        <div class="col-md-4">
                            <label>主题:</label> <input type="text" style="width: 200px;"
                                                      maxlength="30" id="charege-theme" name="filter_LIKES_theme"
                                                      value="${param.filter_LIKES_theme}" class="form-control"
                                                      onchange="fntrim(this)">
                        </div>
                    </div>
                    <br>
                    <div class="row">
                        <div class="col-md-4">
                            <label>申请人:</label> <input type="text" style="width: 200px;"
                                                       maxlength="15" id="charege-info_full_name"
                                                       name="filter_LIKES_full_name"
                                                       value="${param.filter_LIKES_full_name}" class="form-control"
                                                       onchange="fntrim(this)">
                        </div>
                        <div class="col-md-4">
                            <label>经销商编号:</label> <input type="text" style="width: 150px;"
                                                         maxlength="10" id="charege-info_ucode"
                                                         name="filter_LIKES_ucode" value="${param.filter_LIKES_ucode}"
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <label for="selectSystem">所属体系:</label> <select
                                id="selectSystem" class="form-control"
                                name="filter_EQS_systemid" title="">
                            <option value=""
                            ${param.filter_EQS_systemid==""?"selected='selected'":""}>全部
                            </option>
                            <c:forEach items="${systemlist}" var="item">
                                <option value="${item.value}"
                                    ${param.filter_EQS_systemid==item.value?"selected='selected'":""}>${item.name}</option>
                            </c:forEach>
                        </select>
                        </div>
                        <div class="col-md-4">
                            <label for="selectArea">所属大区:</label> <select id="selectArea"
                                                                          onchange="fnGetCompany()" class="form-control"
                                                                          name="filter_EQS_areaId" title="">
                            <option value=""
                            ${param.filter_EQS_areaId==""?"selected='selected'":""}>全部
                            </option>
                            <c:forEach items="${arealist}" var="area">
                                <option value="${area.id}"
                                    ${param.filter_EQS_areaId==area.id?"selected='selected'":""}>${area.name}</option>
                            </c:forEach>
                        </select>
                        </div>
                        <div class="col-md-4">
                            <label for="selectCompany">所属分公司:</label> <select
                                id="selectCompany" class="form-control"
                                name="filter_EQS_companyid" title="">
                            <option value=""
                            ${param.filter_EQS_companyid==""?"selected='selected'":""}>全部
                            </option>
                        </select>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <label for="bussinessType">业务类型:</label> <select
                            ...
                            <option value="9999"
                            ${param.filter_EQS_businessTypeId=="9999"?"selected='selected'":""}>自定义
                            </option>
                        </select>
                        </div>
                        <div class="col-md-8">
                            <label for="bussinessDetailType">业务明细:</label> <select
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <label for="workTask_Status">状态:</label> <select
                                id="workTask_Status" class="form-control"
                                name="filter_EQS_pro_status" title="">
                            <option value=""
                            ${param.filter_EQS_pro_status==""?"selected='selected'":""}>全部
                            </option>
                            <c:forEach items="${statuslist}" var="item">
                                <option value="${item.value}"
                                    ${param.filter_EQS_pro_status==item.value?"selected='selected'":""}>${item.name}</option>
                            </c:forEach>
                        </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div id="pickerStartTime" class="input-group  date">
                                <input style="width: 160px;" type="text"
                                       name="filter_GED_create_time"
                                       value="${param.filter_GED_create_time}"
                            ...
                            <div id="pickerEndTime" class="input-group  date">
                                <input style="width: 160px;" type="text"
                                       name="filter_LED_create_time"
                                       value="${param.filter_LED_create_time}"
                                       class="form-control required valid"> <span
                                    class="input-group-addon"> <i
                                    class="glyphicon glyphicon-calendar"></i>
									</span>
                        ...
                        <div class="col-md-2">
                            <button id="btn_Search" class="btn btn-default a-search"
                                    onclick="fnSearch()" type="submit">查询
                            </button>
                            &nbsp;
                            <button class="btn btn-default a-export" onclick="fnGoAction()"
                                    type="submit">导出
                            </button>
                        </div>
                    </div>
                        ...
                        <c:forEach items="${page.result}" var="item">
                                ...
                                <tr  <c:if test="${item.status == '未审核'|| item.status == '驳回发起人'}"> style="color:blue"  </c:if>  <c:if test="${item.status == '审核中'||item.status == '审核中（驳回）'}"> style="color:green" </c:if>
                                    ...
                                    <td>
                                        ...
                                        <!-- 修改为点击详情直接跳转到表单 原地址/bpm/workspace-viewHistory.do-->
                                        <a href="${tenantPrefix}/bpm/workspace-viewHistoryFrom.do?processInstanceId=${item.processInstanceId}&url=${item.url}" target="_blank">详情</a>
                                        <c:if test="${fn:contains(item.url,'?')}">
                                        	<a href="${tenantPrefix}${item.url}&processInstanceId=${item.processInstanceId}&isPrint=true" target="_blank">打印</a>
                                        </c:if>
	                                     <c:if test="${!fn:contains(item.url,'?')}">
	                                        <a href="${tenantPrefix}${item.url}?processInstanceId=${item.processInstanceId}&isPrint=true" target="_blank">打印</a>
	                                    </c:if>
            ...
            <div class="modal-body">
                <form>
                    ...
                    <div class="input-group userPicker" style="width: 200px;">
                        <input id="_assignee_key_" type="hidden" name="assignee"
                               class="input-medium" value=""> <input type="text"
