    ...
    $(document).delegate('#' + conf.modalId + '_select', 'click', function (e) {
        ...
        if (conf.multiple) {
            ...
            el.each(function (index, item) {
                ...
                approver += "<li style=\"width:140px;float:left;\">"+i+"."+$(item).attr('title')+"</li>";
                if(i == 1){
                	nextLeaderId = $(item).attr('id');
                	nextLeaderName = $(item).attr('title');
                }
                i++;
            });
            /*userPickerElement.children('input[type=hidden]').val(ids.join(','));
            userPickerElement.children('input[type=text]').val(names.join(','));*/
            $("#allLeaderId").val(ids.join(','));
            $("#allLeaderName").val(names.join(','));
            $("#ulapprover").html(approver);
            $("#nextLeaderId").val(nextLeaderId);
            $("#nextLeaderName").val(nextLeaderName);
        } else {
            var el = $('#' + conf.modalId + '_result .label');
            /*userPickerElement.children('input[type=hidden]').val(el.attr('id'));
            userPickerElement.children('input[type=text]').val(el.attr('title'));*/
            $("#allLeaderId").val(el.attr('id'));
            $("#allLeaderName").val(el.attr('title'));
            $("#ulapprover").html("<li style=\"width:140px;float:left;\">1."+el.attr('title')+"</li>");
        }
    });
}
