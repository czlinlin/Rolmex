...
var createUserSelectPicker = function (conf) {
    var ids = "";
    var count = 0;
    var partyStructId = 1;
    conf = conf ? conf : {};
    var defaults = {
        modalId: 'userPickerTwo',
        multiple: false,
...
//     $(document).delegate('.userPickerTwo .input-group-addon', 'click', function(e) {
    $(document).delegate('#' + conf.targetId, 'click', function (e) {
            ...
            if (conf.modalId == 'ccuserPickerTwo') {
                $('#' + conf.modalId + '_username').val("");
                var strIds = "";
            ...
            $('#' + conf.modalId).data('userPickerTwo', $(this).parent());
            $('#' + conf.modalId).modal();
            // doSearch('');
        }
    );
...
// });
    ...
    $(document).delegate('#' + conf.modalId + '_select', 'click', function (e) {
        ...
        var userPickerTwoElement = $('#' + conf.modalId).data('userPickerTwo');
        if (conf.multiple) {
            ...
            userPickerTwoElement.children('input[type=hidden]').val(ids.join(','));
            userPickerTwoElement.children('input[type=text]').val(names.join(','));
            ...
            var el = $('#' + conf.modalId + '_result .label');
            userPickerTwoElement.children('input[type=hidden]').val(el.attr('id'));
            userPickerTwoElement.children('input[type=text]').val(el.attr('title'));
        }
    });
}
