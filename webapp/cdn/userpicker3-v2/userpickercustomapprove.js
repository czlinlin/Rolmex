...
// });
    ...
    $(document).delegate('#' + conf.modalId + '_select', 'click', function (e) {
        ...
        var userPickerElement = $('#' + conf.modalId).data('userPicker').parent();
        if (conf.multiple) {
            ...
            //1 添加   2删除   3  替换
        	if(conf.type == 1){
        		$("#modifyInput").val(conf.type);
        		userPickerElement.children('input[type=hidden]').val(el.attr('id'));
                userPickerElement.children('input[type=text]').val(el.attr('title')); //+','+conf.leaderName
        	}else if(conf.type == 3){
        		$("#modifyInput").val(conf.type);
        		userPickerElement.children('input[type=hidden]').val(el.attr('id'));
